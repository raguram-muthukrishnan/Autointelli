...
import { fetchWebinarBySlug } from '../api';
...
const WebinarDetailPage = () => {
  ...
  useEffect(() => {
    const loadWebinar = async () => {
      try {
        ...
        const data = await fetchWebinarBySlug(slug);
        console.log('Webinar data received:', data);
        if (!data) {
          console.error('No webinar data returned');
        ...
        setError(`Failed to load webinar details: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="loading-state">Loading webinar details...</div>;
  if (error) return <div className="error-state">{error}</div>;
  if (!webinar) return <div className="error-state">Webinar not found.</div>;

  const hasAttributes = webinar.attributes !== undefined;
  const attrs = hasAttributes ? webinar.attributes : webinar;
      ...
      <div className="resource-detail-grid">
        ...
        <div className="resource-main">
          <div className="resource-hero">
            ...
            <div className="hero-badges">
              {is_live && <span className="live-badge-detail">🔴 LIVE NOW</span>}
              {isPast && !is_live && <span className="past-badge">Past Webinar</span>}
            </div>
            <h1>{title}</h1>
          ...
          <div className="metadata-row">
            <div className="metadata-item">
              ...
              <div>
                ...
                <div className="meta-value">
                  {webinarDate ? webinarDate.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : 'TBA'}
                </div>
              </div>
            </div>
            <div className="metadata-item">
            ...
            <div className="metadata-item">
              ...
              <div>
                ...
                <div className="meta-value">{capacity || 'Unlimited'}</div>
              </div>
            </div>
            <div className="metadata-item">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <aside className="resource-sidebar">
          <div className="registration-card">
            <h3>{isPast ? 'Watch Recording' : 'Register Now'}</h3>
            ...
            {isPast ? (
              recording_url ? (
                <a href={recording_url} target="_blank" rel="noopener noreferrer" className="btn-register">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <button disabled className="btn-register">Recording Unavailable</button>
              )
            ) : (
              <>
                {registration_link ? (
                ...
                {isFull && <div className="waitlist-text">This webinar is fully booked.</div>}
              </>
            )}

            <div className="sidebar-info">
              <div className="info-item">
                <strong>Format:</strong> Online Webinar
              </div>
              {is_live && (
                <div className="info-item">
                  <strong>Status:</strong> <span style={{color: '#e53e3e'}}>● Live Now</span>
                </div>
              )}
            </div>
          </div>
